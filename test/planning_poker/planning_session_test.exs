defmodule PlanningPoker.PlanningSessionTest do
  ...
  describe "section editing in voting state" do
    ...
    test "lock_section prevents locking a section locked by another user", %{pid: pid} do
      ...
      assert :gen_statem.call(pid, {:lock_section, "section-0", "user-456"}) ==
    ...
    test "unlock_section prevents unlocking by non-owner", %{pid: pid} do
      ...
      assert :gen_statem.call(pid, {:unlock_section, "section-0", "user-456"}) ==
    ...
    test "update_section_content updates content when locked by user", %{pid: pid} do
      ...
      # Update content
      assert :gen_statem.call(
               pid,
               {:update_section_content, "section-0", "Updated content", "user-123"}
             ) == :ok
    ...
    test "update_section_content fails when not locked by user", %{pid: pid} do
      ...
      result =
        :gen_statem.call(
          pid,
          {:update_section_content, "section-0", "Updated content", "user-456"}
        )
  ...
  describe "issue modification tracking" do
    ...
    test "issue_modified is true when section content is changed", %{pid: pid} do
      ...
      :gen_statem.call(
        pid,
        {:update_section_content, "section-0", "Modified content", "user-123"}
      )
  ...
  describe "section operations outside voting state" do
    test "section operations return error when no current issue", %{pid: pid} do
      ...
      assert :gen_statem.call(pid, {:lock_section, "section-0", "user-123"}) ==
  ...
  describe "magic estimation weight persistence" do
    ...
    test "calculate_weights_from_positions assigns weights correctly", %{pid: pid} do
      ...
      :gen_statem.call(
        pid,
        {:update_issue_position, "marker/1", "unestimated-issues", "estimated-issues", 0}
      )

      # Move issue-2 to position 1 (should get weight 1)
      :gen_statem.call(
        pid,
        {:update_issue_position, "mock-issue-2", "unestimated-issues", "estimated-issues", 1}
      )

      # Move marker/2 to position 2
      :gen_statem.call(
        pid,
        {:update_issue_position, "marker/2", "unestimated-issues", "estimated-issues", 2}
      )

      # Move issue-1 to position 3 (should get weight 2)
      :gen_statem.call(
        pid,
        {:update_issue_position, "mock-issue-1", "unestimated-issues", "estimated-issues", 3}
      )

      # Move marker/5 to position 4
      :gen_statem.call(
        pid,
        {:update_issue_position, "marker/5", "unestimated-issues", "estimated-issues", 4}
      )

      # Move issue-3 to position 5 (should get weight 5)
      :gen_statem.call(
        pid,
        {:update_issue_position, "mock-issue-3", "unestimated-issues", "estimated-issues", 5}
      )
    ...
    test "complete_estimation with estimated issues creates update tasks", %{pid: pid} do
      ...
      :gen_statem.call(
        pid,
        {:update_issue_position, "marker/2", "unestimated-issues", "estimated-issues", 0}
      )
    ...
    test "issues before first marker are not assigned weights", %{pid: pid} do
      ...
      :gen_statem.call(
        pid,
        {:update_issue_position, "mock-issue-1", "unestimated-issues", "estimated-issues", 0}
      )

      # Then add a marker after it
      :gen_statem.call(
        pid,
        {:update_issue_position, "marker/2", "unestimated-issues", "estimated-issues", 1}
      )

      # And another issue after the marker
      :gen_statem.call(
        pid,
        {:update_issue_position, "mock-issue-2", "unestimated-issues", "estimated-issues", 2}
      )
